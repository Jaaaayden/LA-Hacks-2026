...
- **Anthropic Codex Haiku 4.5** via `client.messages.parse(output_format=PydanticClass)` for structured intent parsing
...
User input
   │
   ▼
create_query_session() ─▶ parse_intent() ─▶ Codex
   │  (DONE)              (DONE)
   ▼
queries collection ──────────────────┐
...
- **LLM provider**: Anthropic Codex Haiku 4.5. (Switched from Opus 4.7 for cost; quality on JSON extraction is identical.)
